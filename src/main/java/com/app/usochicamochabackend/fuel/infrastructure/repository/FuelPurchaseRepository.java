...
public interface FuelPurchaseRepository extends JpaRepository<FuelPurchaseEntity, Long> {
    ...
    @Query("SELECT f.fuelTypeId, COALESCE(SUM(f.totalCalculado), 0) FROM FuelPurchaseEntity f "
            + "WHERE f.fechaCompra BETWEEN :inicio AND :fin GROUP BY f.fuelTypeId")
    List<Object[]> sumTotalCalculadoPorTipoBetween(@Param("inicio") Timestamp inicio, @Param("fin") Timestamp fin);
    ...
    @Query("SELECT COALESCE(SUM(f.cantidad), 0) FROM FuelPurchaseEntity f "
            + "WHERE f.fechaCompra BETWEEN :inicio AND :fin")
    BigDecimal sumCantidadBetween(@Param("inicio") Timestamp inicio, @Param("fin") Timestamp fin);

    long countByDiscrepanciaValorTrueAndFechaCompraBetween(Timestamp inicio, Timestamp fin);

    // Tipos de combustible con al menos una compra alguna vez (sin importar fecha) — usado
    // para no mostrar en Saldos/Conciliación tipos del catálogo que la organización nunca
    // ha manejado (fuel_inventory se siembra para los 4 tipos desde el inicio, los usen o no).
    @Query("SELECT DISTINCT f.fuelTypeId FROM FuelPurchaseEntity f")
